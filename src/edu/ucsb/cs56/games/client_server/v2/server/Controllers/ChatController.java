...
public class ChatController extends ServiceController {
    ...
    public ChatController(JavaServer server) {
     ...
     */
    @Override
    public void switchServices(ClientNetworkController client, ServiceController service){
        if(client.client.getLocation() == service.id)
            return;
        client.currentService.removeClient(client);
        server.broadcastMessage("MOVED[" + client.client.getId() + "]" + service.id);
